...
AS $$
DECLARE
  caller_subject TEXT := COALESCE(auth.uid()::TEXT, '');
...
AS $$
BEGIN
  IF COALESCE(auth.uid()::TEXT, '') NOT IN ('service_role', 'project-admin-with-api-key')
    AND COALESCE(current_setting('request.jwt.claim.role', TRUE), '') NOT IN ('service_role', 'project-admin-with-api-key')
  THEN
